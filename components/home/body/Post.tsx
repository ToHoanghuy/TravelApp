...
function Post({ post, navigation }: { post: any, navigation: any }) {
  ...
  function PostStats() {
    ...
    // Check if the user has already liked the post when the component loads
    useEffect(() => {
    ...
    async function handleLike() {
      ...
      try {
        ...
        if (!data.isSuccess) {
          ...
          setTotalLikes((prevData: number) => (liked ? prevData + 1 : prevData - 1));
          setLiked(!liked);
          console.log('Error reacting to post:', data.error);
        } else {
          console.log('Successfully reacted to post:', data.data);
        }
      } catch (error) {
      ...
      try {
        ...
        if (result.action === Share.sharedAction) {
          if (result.activityType) {
            // Shared via a specific activity type
    ...
    // Get hashtags from post content or tags if available
    const getHashtags = () => {
      const hashtags = post?.hashTags || [];

      // If no hashtags, try to extract from content
